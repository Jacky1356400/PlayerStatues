...
public class GuiSculpt extends GuiScreenPlus  {
	...
	public GuiSculpt(World world, int px, int py, int pz, final EntityPlayer entityplayer,int facing) {
		...
		block=world.getBlockState(new BlockPos(worldX, worldY, worldZ)).getBlock();
		blockMeta=world.getBlockState(new BlockPos(worldX, worldY, worldZ)).getBlock().getMetaFromState(world.getBlockState(new BlockPos(worldX, worldY, worldZ)));
		...
		addChild(new GuiExButton(9, 199, 209, 20, "Sculpt!") {
			...
			public void onClick() {
		    	...
		    	try {
		    		Packet asculpt = PlayerStatues.packet.create(Packets.SCULPTURE_CREATION)
