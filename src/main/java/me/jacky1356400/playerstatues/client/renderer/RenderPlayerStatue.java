...
import me.jacky1356400.playerstatues.client.model.ModelBipedStatue;
...
public class RenderPlayerStatue extends RendererLivingEntity
{
	...
	public void renderItemInRightArm(EntityStatuePlayer player)
	{
		...
		renderItemInArm(player, player.inventory.getStackInSlot(0), player.getPose().itemRightA);
	...
	public void renderItemInLeftArm(EntityStatuePlayer player)
	{
		...
		renderItemInArm(player, player.inventory.getStackInSlot(1), 1.0f - player.getPose().itemLeftA);
	...
	{
		model.bipedRightArm.rotateAngleX = multiplier * (0 - 180 * player.getPose().armRightB);
		model.bipedRightArm.rotateAngleY = multiplier * (90 - 135 * player.getPose().armRightA);
		model.bipedRightArm.rotateAngleZ = multiplier * 0;
		model.bipedLeftArm.rotateAngleX = multiplier * (+180 * (1.0f - player.getPose().armLeftB));
		model.bipedLeftArm.rotateAngleY = multiplier * (270 - 135 * player.getPose().armLeftA);
		model.bipedLeftArm.rotateAngleZ = multiplier * 180;
		model.bipedRightLeg.rotateAngleX = multiplier * (120 - 240 * player.getPose().legRightB);
		model.bipedRightLeg.rotateAngleY = multiplier * (90 - 90 * player.getPose().legRightA);
		model.bipedRightLeg.rotateAngleZ = 0;
		model.bipedLeftLeg.rotateAngleX = multiplier * (120 - 240 * player.getPose().legLeftB);
		model.bipedLeftLeg.rotateAngleY = multiplier * (-90 + 90 * player.getPose().legLeftA);
		model.bipedLeftLeg.rotateAngleZ = 0;
		model.bipedHead.rotateAngleX = multiplier * (-45 + 90 * player.getPose().headB);
		model.bipedHead.rotateAngleY = multiplier * (45 - 90 * player.getPose().headA);
	...
	{
		double cos = Math.cos(leg.rotateAngleX);
		if(cos < 0)
			cos = 0;
	...
	public void doRender(EntityLivingBase par1EntityLivingBase, double x, double y, double z, float unk, float frame)
	{
		...
		GL11.glRotatef(-45 + player.getPose().bodyA * 90, 0.0f, 1.0f, 0.0f);
		GL11.glTranslatef(0.0f, -player.yOffset, 0.0f);
		GL11.glRotatef(-30 + player.getPose().bodyB * 60, 1.0f, 0.0f, 0.0f);
