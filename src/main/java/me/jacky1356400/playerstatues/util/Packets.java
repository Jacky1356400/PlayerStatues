...
public class Packets extends MessageHandlerBase {
	...
	public void onSculptureCreation(Packet packet, EntityPlayer player) throws IOException {
		...
		if(!PlayerStatues.canSculpt(block, player.worldObj,x,y,z)) return;
		...
		player.world.setBlock(x, y, z, PlayerStatues.statue,face,3);
		
		TileEntity tileEntity = player.world.getTileEntity(x, y, z);
		...
		player.world.setBlock(x, y+1, z, PlayerStatues.statue, face|4,3);
		player.world.notifyBlocksOfNeighborChange(x, y, z, PlayerStatues.statue);
